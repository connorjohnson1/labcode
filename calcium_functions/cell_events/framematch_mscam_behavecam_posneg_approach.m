function[cellfreq, startstopframes, totaltime] = framematch_mscam_behavecam_posneg_approach(approach, behavior, cell_events, timestamp)

%CJ ACM Lab 7/9/2020
%
%The purpose of this function is to find the frames of our ca2+ data that
%align to our logical behavior data. This code is slightly altered from the
%original version so that instead of looking only at the time spent
%interating, we look at a range of time of +/- n seconds before and after
%an interaction, n = approach.
%
%INPUTS:
%
%Approach = How many seconds you want to look at before and after an 
%interaction starts.
%
...
%calculating frequency
    ...
    if size(soc,1) > 0
        ...
        %get negative approach frames
        start = startstop;
        start(:,2) = startstop(:,1);
        start(:,1) = startstop(:,1) - t;
        start = start(start(:,1) > 0,:);
        %get positive approach frames
        start(:,2) = start(:,1) + t;
        start = start(start(:,2) < size(btime,1),:);
        startstop = start;
