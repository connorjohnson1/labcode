function[trans_freq, startstopframes, totaltime] = get_transient_length(behavior, cell_events, timestamp)
...
%calculating frequency
    ...
    if size(soc,1) > 0
        ...
        for i = 1:size(startstop,1)
            startstoptimes(i, 1) = mstime(startstop(i,1));
            startstoptimes(i, 2) = mstime(startstop(i,2));
        end
        ...
        trans_freq = (size(startstop,1)/(max(mstime)/1000))*100;
