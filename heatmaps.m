...
%    idx_cells = zscored_cell_filt(:,idx);
    ...
    for ii = 2:3
        ...
        for iii = 1:size(msframes,1)
            x = raw_trace_filt(msframes(iii,1):msframes(iii,2),idx);
            cells(iii).avg = x;
        end
        %average traces
        avgca = cells(1).avg;
...
avgca = horzcat(traces.center);
   ...
   % sortavgca = sortavgca(:,xxx);
    ...
    caxis([0 1]);
    title('Periphery Selective Cells')
    ylabel('Cell #')
    xlabel('Time From Entering Center (seconds)')
    pl = line([0,0], [size(o,2),0]);
    pl.Color = 'Black';
    pl.LineWidth = 1.5 ;
    colorbar